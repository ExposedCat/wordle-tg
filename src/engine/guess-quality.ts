...
export function qualityPoints(
	...
	return clamp(
		Math.round(100 - (50 * (remaining - 1)) / (averageRemaining - 1)),
		0,
		100,
	);
}
...
export function possibleSecretWords(
	...
	const feedback = previousGuesses.map((guess) =>
		scoreKey(scoreGuess(answer, guess)),
	);

	return answers.filter((candidate) =>
		previousGuesses.every(
			(guess, index) =>
				scoreKey(scoreGuess(candidate, guess)) === feedback[index],
		),
	);
}

function actualRemainingForGuess(
	answer: string,
	guess: string,
...
function expectedRemainingForGuess(
	possible: readonly string[],
	guess: string,
): number {
	const patternCounts = new Map<string, number>();
...
function averageRemainingForState(possible: readonly string[]): number {
	...
	for (const guess of guesses)
		total += expectedRemainingForGuess(possible, guess);
