...
const DATA_DIR = Deno.env.get("WORDLE_DATA_DIR") ?? join(Deno.cwd(), "data");
...
function loadJson(
	language: WordLanguage,
	length: number,
): { answers: string[]; valid: Set<string> } {
	const path = join(DATA_DIR, `${language}-${length}.json`);
	const parsed = JSON.parse(Deno.readTextFileSync(path)) as Partial<WordJson>;
	if (
		parsed.language !== language ||
		parsed.length !== length ||
		!Array.isArray(parsed.valid) ||
		!Array.isArray(parsed.possible)
	) {
...
const ANSWERS_BY_LANGUAGE_AND_LENGTH: Record<
	WordLanguage,
	Record<number, string[]>
> = { en: {}, ru: {} };
const VALID_BY_LANGUAGE_AND_LENGTH: Record<
	WordLanguage,
...
export const ANSWERS: string[] =
	ANSWERS_BY_LANGUAGE_AND_LENGTH.en[DEFAULT_WORD_LENGTH];
export const ANSWERS_RU: string[] =
	ANSWERS_BY_LANGUAGE_AND_LENGTH.ru[DEFAULT_WORD_LENGTH];
...
export function isValidWord(
	...
	return (
		VALID_BY_LANGUAGE_AND_LENGTH[language][length]?.has(word.toLowerCase()) ??
		false
	);
}

export function answersForLanguage(
	language: WordLanguage = "en",
	length = DEFAULT_WORD_LENGTH,
): readonly string[] {
...
export function pickAnswer(
	...
	const answers =
		ANSWERS_BY_LANGUAGE_AND_LENGTH[language][length] ??
		ANSWERS_BY_LANGUAGE[language];
