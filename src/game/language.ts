...
export function isLanguageWord(
	word: string,
	...
	return (
		normalized.length === length && WORD_PATTERNS[language].test(normalized)
	);
}

export function isGuessText(
	text: string,
	length = DEFAULT_WORD_LENGTH,
): boolean {
	return (
		isSupportedWordLength(length) &&
		new RegExp(`^\\p{Letter}{${length}}$`, "u").test(text)
	);
}
