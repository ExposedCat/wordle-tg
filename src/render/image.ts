...
export async function renderCompareSticker(left: CompareStickerPlayer, right: CompareStickerPlayer): Promise<Buffer> {
  ...
  for (const metric of metrics) {
    const leftWins = metric.leftScore !== null && metric.rightScore !== null && metric.leftScore > metric.rightScore;
    const rightWins = metric.leftScore !== null && metric.rightScore !== null && metric.rightScore > metric.leftScore;
    const tieWithValue =
    ...
    drawMetricCell(ctx, rowX, y, 132, rowH, metric.left, leftWins || tieWithValue);
    drawMetricCenter(ctx, rowX + 140, y, 176, rowH, metric.label);
    drawMetricCell(ctx, rowX + 324, y, 132, rowH, metric.right, rightWins || tieWithValue);
    y += rowH + gap;
  }
...
function compareMetrics(left: StatsRow, right: StatsRow): {
  label: string;
  left: string;
  right: string;
...
async function drawCompareAvatar(
  ...
  if (player.avatar) {
    try {
      const image = await loadImage(player.avatar);
      ...
      ctx.clip();
      ctx.drawImage(image, sx, sy, size, size, centerX - radius, centerY - radius, radius * 2, radius * 2);
      ctx.restore();
      return;
    } catch {
